...
exports.AIDataVisualization = void 0;
...
class AIDataVisualization {
    constructor(config) {
        this.state = types_1.VisualizationState.IDLE;
    ...
    resolveContainer(container) {
        if (typeof container === 'string') {
            ...
            if (!element) {
                throw new types_1.AIDataVisualizationError(types_1.ErrorType.CONTAINER_NOT_FOUND, `Container element not found: ${container}`);
            }
            return element;
        }
        return container;
    ...
    validateConfig() {
        if (!this.config.apiDescription) {
            throw new types_1.AIDataVisualizationError(types_1.ErrorType.INVALID_API_DESCRIPTION, 'API description is required');
        }
        if (typeof this.config.chatCompletion !== 'function') {
            throw new types_1.AIDataVisualizationError(types_1.ErrorType.CHAT_COMPLETION_FAILED, 'chatCompletion must be a function');
        }
        if (typeof this.config.apiRequest !== 'function') {
            throw new types_1.AIDataVisualizationError(types_1.ErrorType.API_REQUEST_FAILED, 'apiRequest must be a function');
        }
    }
    /**
    ...
    setupEventListeners() {
        ...
        textarea.addEventListener('input', () => {
            generateBtn.disabled = !textarea.value.trim() || this.state === types_1.VisualizationState.GENERATING;
        });
        // Initial state
    ...
    setupMessageListener() {
        this.messageListener = (event) => {
            ...
            if (data.type === 'API_REQUEST' && data.requestId && data.url) {
                ...
                this.setState(types_1.VisualizationState.ERROR);
    ...
    async generateVisualization() {
        ...
        try {
            const isImprovement = this.lastGeneratedCode !== null && this.state === types_1.VisualizationState.DISPLAYING;
            ...
            this.setState(types_1.VisualizationState.GENERATING);
            this.hideError();
            ...
            if (!htmlResponse || !htmlResponse.trim()) {
                throw new types_1.AIDataVisualizationError(types_1.ErrorType.INVALID_HTML_RESPONSE, 'AI returned empty response');
            }
...
<script>window.addEventListener('error',e=>{window.parent.postMessage({type:'IFRAME_ERROR',message:e.message,stack:e.error&&e.error.stack},'*');});window.addEventListener('unhandledrejection',e=>{window.parent.postMessage({type:'IFRAME_ERROR',message:e.reason?e.reason.message||String(e.reason):'Unhandled rejection',stack:e.reason&&e.reason.stack},'*');});</script>`;
    ...
    clearVisualization() {
        ...
        this.setState(types_1.VisualizationState.IDLE);
        this.hideError();
        if (this.iframe) {
            this.iframe.srcdoc = '';
        }
    ...
    setState(state) {
        ...
        const textarea = this.container.querySelector('.ai-data-viz__textarea');
        switch (state) {
            case types_1.VisualizationState.GENERATING:
                generateBtn.disabled = true;
                btnText.textContent = 'Generating...';
                btnSpinner.style.display = 'block';
                textarea.disabled = true;
                break;
            case types_1.VisualizationState.DISPLAYING:
                generateBtn.disabled = !textarea.value.trim();
                btnText.textContent = 'Improve Visualization';
                btnSpinner.style.display = 'none';
                textarea.disabled = false;
                break;
            case types_1.VisualizationState.IDLE:
            case types_1.VisualizationState.ERROR:
                generateBtn.disabled = !textarea.value.trim();
...
exports.AIDataVisualization = AIDataVisualization;
